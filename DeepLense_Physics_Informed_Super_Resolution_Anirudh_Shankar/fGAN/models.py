...
class SersicProfiler(torch.nn.Module):
    def __init__(self, resolution, device, discriminators, feature_extractor, sersic_args = [1, 1, 0.25], vdl_weight=1, multi_scale_loss_scales = [1, 0.5, 0.25], beta = 1e-3) -> None:
        """
        ...
        :param discriminators: List of discriminator modules, corresponding to the multi_scale_loss_scales
        :param feature_extractor: The feature extractor (truncated classifier) module
        ...
        self.adversarial_loss = torch.nn.CrossEntropyLoss()
        self.beta = beta
    ...
    def multi_scale_loss(self, alpha, LR, resolution, R_sersic = None):
        ...
        for scale in self.scale_list:
            if scale == 1:
                ...
                S_disc = self.discriminators[0](S_lens)
                I_disc = self.discriminators[0](I_lens)
                losses += torch.nn.functional.mse_loss(source_interpolated, S_lens)# + self.beta*self.adversarial_loss(S_disc, torch.ones_like(S_disc))
                losses += self.vdl_weight*self.variation_density_loss(alpha)
                losses += torch.nn.functional.mse_loss(self.feature_extractor(LR_interpolated), self.feature_extractor(I_lens))# + self.beta*self.adversarial_loss(I_disc,torch.ones_like(I_disc))

                individual_losses['source_0'] = torch.nn.functional.mse_loss(source_interpolated, S_lens)# + self.beta*self.adversarial_loss(S_disc, torch.ones_like(S_disc)))
                individual_losses['image_0'] = torch.nn.functional.mse_loss(LR_interpolated, I_lens)
                individual_losses['alpha_0'] = self.vdl_weight*self.variation_density_loss(alpha)# + self.beta*self.adversarial_loss(I_disc,torch.ones_like(I_disc)))

                S_disc = self.discriminators[0](S_lens.detach())
                source_disc = self.discriminators[0](source_interpolated.detach())
                ad_losses += self.adversarial_loss(S_disc, torch.zeros_like(S_disc)) + self.adversarial_loss(source_disc, torch.zeros_like(source_disc))
                
                I_disc = self.discriminators[0](I_lens.detach())
                LR_disc = self.discriminators[0](LR_interpolated.detach())
                ad_losses += self.adversarial_loss(I_disc, torch.zeros_like(I_disc)) + self.adversarial_loss(LR_disc, torch.zeros_like(LR_disc))
                ...
                S_disc = self.discriminators[1](S_lens)
                losses += torch.nn.functional.mse_loss(source_profile_regenerated, S_lens) + self.beta*self.adversarial_loss(S_disc, torch.ones_like(S_disc))
                individual_losses['source_1'] = torch.nn.functional.mse_loss(source_profile_regenerated, S_lens) + self.beta*self.adversarial_loss(S_disc, torch.ones_like(S_disc))

                S_disc = self.discriminators[1](S_lens.detach())
                source_disc = self.discriminators[1](source_profile_regenerated.detach())
                ad_losses += self.adversarial_loss(S_disc, torch.zeros_like(S_disc)) + self.adversarial_loss(source_disc, torch.zeros_like(source_disc))
                ...
                S_disc = self.discriminators[2](S_lens)
                losses += torch.nn.functional.mse_loss(source_interpolated, S_lens) + self.beta*self.adversarial_loss(S_disc, torch.ones_like(S_disc))
                individual_losses['source_2'] = torch.nn.functional.mse_loss(source_interpolated, S_lens) + self.beta*self.adversarial_loss(S_disc, torch.ones_like(S_disc))

                S_disc = self.discriminators[2](S_lens.detach())
                source_disc = self.discriminators[2](source_interpolated.detach())
                ad_losses += self.adversarial_loss(S_disc, torch.zeros_like(S_disc)) + self.adversarial_loss(source_disc, torch.zeros_like(source_disc))
        return losses, individual_losses, ad_losses
...
class Classifier(torch.nn.Module):
    def __init__(self, in_shape, in_channels=1) -> None:
        super(Classifier, self).__init__()
        self.conv1 = torch.nn.Conv2d(1*in_channels,16*in_channels,kernel_size=3, stride=2, padding=1)
        self.relu = torch.nn.ReLU()
        self.conv2 = torch.nn.Conv2d(16*in_channels,32*in_channels,kernel_size=3, stride=2, padding=1)
        self.flat = torch.nn.Flatten()
        self.fc1 = torch.nn.Linear(32*in_channels*int(np.ceil(in_shape/4))**2, 128)
        # self.fc2 = torch.nn.Linear(128, out_shape)
