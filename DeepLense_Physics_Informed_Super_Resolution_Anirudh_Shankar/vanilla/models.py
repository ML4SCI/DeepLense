    ...
    def multi_scale_loss(self, alpha, LR, resolution, R_sersic = None):
        ...
        for scale in self.scale_list:
            if scale == 1:
                ...
                source_profile_regenerated, S_lens, I_lens = self.create_lensing(LR, alpha_LR, alpha, resolution, resolution/2, 2, R_sersic=R_sersic)
                ...
                alpha_LR = torch.nn.functional.interpolate(alpha, scale_factor=0.5, mode='bicubic')
                source_profile_regenerated, S_lens, _ = self.create_lensing(LR, alpha_LR, alpha_LR, resolution, resolution, 1, R_sersic=R_sersic)
                ...
                alpha_LR = torch.nn.functional.interpolate(alpha, scale_factor=0.5, mode='bicubic')
                source_profile_regenerated, S_lens, _ = self.create_lensing(LR, alpha_LR, alpha_interpolated, resolution, resolution/LR_scale, LR_scale, R_sersic=R_sersic)
