...
for i in tqdm(range(num_sim)):
    ...
    np.save(directory+'data_model_2/cdm/cdm_sim_%d'%i,File)
    lens.simple_sim_2(128,2)
    File = lens.image_real
    np.save(directory+'data_model_2/cdm_HR/cdm_HR_sim_%d'%i,File)
    lens.get_alpha(128,0.0505)
    np.save(directory+'data_model_2/cdm_HR/cdm_alpha_sim_%d'%i,lens.alpha)
