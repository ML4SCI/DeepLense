...
class SersicProfiler(torch.nn.Module):
    ...
    def create_source(self, alpha_r, resolution, LR):
        """
    ...
    def create_lensing(self, LR, alpha_r, alpha_r_interpolated, resolution, resolution_, magnification, R_sersic = None):
        B, _, x, y = LR.shape
        arcsec_bound = resolution*x/2 #
        pos_x = torch.linspace(-arcsec_bound, arcsec_bound, x).to(self.device)
        pos_y = torch.linspace(-arcsec_bound, arcsec_bound, y).to(self.device)
    ...
    def multi_scale_loss(self, alpha, LR, resolution, R_sersic = None):
        ...
        for scale in self.scale_list:
            if scale == 1:
                ...
                alpha_LR = torch.nn.functional.interpolate(alpha, scale_factor=0.5, mode='bicubic')
                source_profile_regenerated, S_lens, __, _ = self.create_lensing(LR, alpha_LR, alpha_LR, resolution, resolution, 1, R_sersic=R_sersic)
    ...
    def get_sample(self, alpha, LR, plot, R_sersic = None):
        ...
        if not plot: return I_lens, LR, source_profile_regenerated
    ...
    def approximate_center(self, intensity_profile):
        ...
        intensity_profile = intensity_profile*(intensity_profile > self.cutoff_factor) - self.cutoff_factor
        # cutoff = max_value * self.cutoff_factor
        width = torch.zeros_like(max_index)
        for i in range(intensity_profile.shape[0]):
            low, high = max_index[i].clone(), max_index[i].clone()
            while intensity_profile[i][low] > 0: low -= 1
            while intensity_profile[i][high] > 0: high += 1
            # mean_indices[i] = ((low + high)//2 + max_index[i])//2
            width[i] = high - low
        return max_index, width
...
class SISR(torch.nn.Module):
    def __init__(self, magnification, n_mag, residual_depth, in_channels=1, out_channels=1, latent_channel_count=64):
        ...
        self.conv3 = torch.nn.Conv2d(in_channels=self.latent_channel_count,out_channels=out_channels,kernel_size=3,padding=1)
...
class Classifier(torch.nn.Module):
    def __init__(self, in_shape, out_classes, model, in_channels=1, lr=False):
        """
        A residual classifier module

        :param in_shape: Image dimensions (assuming square images)
        :param out_classes: Number of prediction classes
        :param model: Data model number (1,2 or 3)
        :param in_channels: Number of channels in the images
        :param lr: True if classifier inputs are LR images; used to decide fc1's shape
        """
        super(Classifier, self).__init__()
        img_shape = in_shape
        self.conv1 = torch.nn.Conv2d(in_channels, 8, kernel_size=5, padding=2, stride=1)
        self.relu = torch.nn.ReLU()
        self.b1 = self.make_residual_block(8)
        self.conv2 = torch.nn.Conv2d(8, 16, kernel_size=4, padding=2, stride=2)
        img_shape = int(np.ceil((img_shape - 4 + 4)/2+1))
        self.b2 = self.make_residual_block(16)
        self.conv3 = torch.nn.Conv2d(16, 24, kernel_size=4, padding=2, stride=2)
        img_shape = int(np.ceil((img_shape - 4 + 4)/2+1))
        self.b3 = self.make_residual_block(24)
        self.conv4 = torch.nn.Conv2d(24, 32, kernel_size=4, padding=2, stride=2)
        img_shape = int(np.ceil((img_shape - 4 + 4)/2))
        self.b4 = self.make_residual_block(32)
        self.conv5 = torch.nn.Conv2d(32, 32, kernel_size=4, padding=2, stride=2)
        img_shape = int(np.ceil((img_shape - 4 + 4)/2+1))

        self.flatten = torch.nn.Flatten()
        if not lr:
            if model == 1:
                self.fc1 = torch.nn.Linear(32*img_shape*img_shape, 256)
            else:
    ...
    def make_residual_block(self, channels):
        """
        Constructs and returns a residual block with the specified channels
        """
        return torch.nn.Sequential(
            torch.nn.Conv2d(channels, channels, kernel_size=3, padding=1, stride=1),
