...
parser.add_argument(
    ...
    metavar="XXX/YYY",
    type=str,
    default="self_supervised/config/crossformer_byol.yaml",
...
def main():
    ...
    default_dataset_setup = DefaultDatasetSetupSSL(dataset_name=dataset_name, image_size=image_size)
    ...
    summary(
        transformer, input_size=(train_config["batch_size"], 1, image_size, image_size)
    )

    # backbone = nn.Sequential(*list(transformer.children())[:-1])
    backbone = transformer
    model = BYOLTransformer(backbone, num_ftrs=in_features, hidden_dim=512, out_dim=256)
...
if __name__ == "__main__":
    ...
    """python byol_transformer.py 

    python examples/byol/byol_transformer.py --train_config_path config/crossformer_byol.yaml
