...
from models._momentum import _MomentumEncoderMixin
from models.modules.head import BYOLProjectionHead
from utils.util import deactivate_requires_grad
import copy
...
class BYOLSingleChannel(nn.Module, _MomentumEncoderMixin):
    """Implementation of the BYOL architecture.
    ...
    def __init__(
        ...
        super(BYOLSingleChannel, self).__init__()
...
class FinetuneModelByol(nn.Module):
    def __init__(self, backbone: BYOLSingleChannel, head: nn.Module):
        super().__init__()

        self.model = copy.deepcopy(backbone)
        print(self.model.prediction_head)
        self.model.prediction_head = head

        deactivate_requires_grad(self.model.backbone)
        deactivate_requires_grad(self.model.projection_head)
...
class BYOLTransformer(BYOL):
    def __init__(
        ...
        BYOL.__init__(self, backbone, num_ftrs, hidden_dim, out_dim, m)
