...
# file adapted from: https://github.com/lucidrains/vit-pytorch/blob/main/vit_pytorch/twins_svt.py
# helper methods
...
class LocalAttention(nn.Module):
    ...
    def forward(self, fmap):
        ...
        q, k, v = map(
            lambda t: rearrange(t, "b (h d) p1 p2 -> (b h) (p1 p2) d", h=h), (q, k, v)
        )
        ...
        out = rearrange(
            out,
            "(b x y h) (p1 p2) d -> b (h d) (x p1) (y p2)",
            h=h,
            x=x,
            y=y,
            p1=p,
        ...
        *,
        ...
        for prefix in ("s1", "s2", "s3", "s4"):
            ...
            layers.append(
                nn.Sequential(
                    PatchEmbedding(
                        dim=dim, dim_out=dim_next, patch_size=config["patch_size"]
                    ),
                    Transformer(
                        dim=dim_next,
                        depth=1,
