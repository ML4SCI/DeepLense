...
data_loader = DataLoader(dataset=dataset, batch_size=5, shuffle=config.data.shuffle)
#print(data_loader)
dataset_1 = CustomDataset_1(root_dir=config.data.folder, config=config)
data_loader_1 = DataLoader(dataset=dataset_1, batch_size=1, shuffle=config.data.shuffle)
...
for batch in data_loader:
    ...
    sample_images = diffusion.sample_conditional(model, 5, v1, v2, v3, v4, cfg_scale=0)
    ...
    plt.savefig(os.path.join("plots", f"all_variables_gen.png"), dpi=300, bbox_inches='tight')
