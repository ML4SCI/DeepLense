...
class Patches(torch.nn.Module):
    """
    Layer which breaks the input image into patches
...
class PatchEncoder(torch.nn.Module):
    """
    Layer to produce embeddings from image patches
    using a dense layer and positional information
    """
    ...
    def forward(self, patch):
        positions = torch.arange(0, self.num_patches, dtype=torch.long).to(
...
class Transformer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.attention = MultiheadAttention(
            projection_dim, num_heads=num_heads, dropout=dropout_rate
        )
    ...
    def forward(self, encoded_patch):
        ...
        x2 = attention_output + encoded_patch
        x3 = self.norm2(x2)
        x3 = self.ffn(x3)
        output = x3 + x2
        return output
