...
class iBOTLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch, student_mask):
        """
        Cross-entropy between softmax outputs of the teacher and student networks.
        """
        student_output, student_local_cls = student_output
    ...
    def update_center(self, teacher_cls, teacher_patch):
        ...
        """
        cls_center = torch.mean(teacher_cls, dim=0, keepdim=True)
        self.center = self.center * self.center_momentum + cls_center * (1 - self.center_momentum)

        patch_center = torch.mean(teacher_patch.mean(1), dim=0, keepdim=True)
